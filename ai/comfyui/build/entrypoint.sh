...
# The mkdir/cp above (user/, custom_nodes/comfyui-manager/) run as root, but the
# actual comfy process below drops to PUID:PGID — without this, ComfyUI-Manager
# can't write its own state (db, __manager dirs) into what it just created.
chown -R "$PUID:$PGID" /data/config/comfy

# Drop root privileges for the actual comfy process, so files it creates
# (output images, cache) are owned by PUID:PGID instead of root.
exec gosu "${PUID}:${PGID}" "$@"
