...
# Several extensions' install.py scripts pip-install packages without pinning
# numpy, and pip's per-package resolver upgrades numpy to 2.x to satisfy them.
# scikit-image (and its compiled geometry.pyx extension) is only ABI-compatible
# with numpy<2, so re-pin it back down after all extension deps are installed.
...
# Drop root privileges for the actual webui process, so files it creates
# (output images, cache, hash lookups) are owned by PUID:PGID instead of root.
exec gosu "${PUID}:${PGID}" "$@"
